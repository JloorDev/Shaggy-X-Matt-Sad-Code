	...
	public function new(curStage)
	{
		...
		switch (curStage)
		{
			...
			case 'boxingsad':
				...
				curStage = 'boxingsad';
				var bg:FNFSprite = new FNFSprite(-400, -220).loadGraphic(Paths.image('backgrounds/' + curStage + '/bg_boxn'));
				bg.antialiasing = (!Init.trueSettings.get('Disable Antialiasing'));
				...
				starz = new FNFSprite(200, 0);
				starz.frames = Paths.getSparrowAtlas('backgrounds/' + curStage + '/starz');
				starz.animation.addByPrefix('weee', "shootinstarz", 24, false);
				starz.antialiasing = (!Init.trueSettings.get('Disable Antialiasing'));
				starz.scrollFactor.set(0.2, 0.2);
				starz.updateHitbox();
				add(starz);
				starz.animation.play('weee', true);

				var bgr:FNFSprite = new FNFSprite(-810, -380).loadGraphic(Paths.image('backgrounds/' + curStage + '/bg_boxr'));
				bgr.antialiasing = (!Init.trueSettings.get('Disable Antialiasing'));
				bgr.scrollFactor.set(1, 1);
				bgr.active = false;
	...
	public function stageUpdate(curBeat:Int, boyfriend:Boyfriend, gf:Character, dadOpponent:Character)
	{
		...
		switch (PlayState.curStage)
		{
			...
			case "boxingsad":
				...
				if (curBeat == 47) // start
				{
					var uno:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menus/base/uno'));
					uno.scrollFactor.set();
					uno.updateHitbox();
					uno.screenCenter();
					add(uno);
					FlxTween.tween(uno, {alpha: 0}, 0.6, {
						ease: FlxEase.expoOut
					});
				}
				if (curBeat == 48)
				{
					var dos:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menus/base/dos'));
					dos.scrollFactor.set();
					dos.updateHitbox();
					dos.screenCenter();
					add(dos);
					FlxTween.tween(dos, {alpha: 0}, 0.6, {
						ease: FlxEase.expoOut
					});
				}
				if (curBeat == 49)
				{
					var tres:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menus/base/tres'));
					tres.scrollFactor.set();
					tres.updateHitbox();
					tres.screenCenter();
					add(tres);
					FlxTween.tween(tres, {alpha: 0}, 0.6, {
						ease: FlxEase.expoOut
					});
				}
				if (curBeat == 50)
				{
					var gooo:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menus/base/cuatro'));
					gooo.scrollFactor.set();
					gooo.updateHitbox();
					gooo.screenCenter();
					add(gooo);
					FlxTween.tween(gooo, {alpha: 0}, 0.6, {
						ease: FlxEase.expoOut
					});
				}
