...
class PlayState extends MusicBeatState
{
			...
			if (!PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
			{
				...
				switch (dadOpponent.curCharacter)
				{
					...
					case 'senpai-angry':
						...
						getCenterX = char.getMidpoint().x - 100;
					case 'shaggy':
						getCenterY = char.getMidpoint().y - 110;
						getCenterX = char.getMidpoint().x - 180;
				}

				camFollow.setPosition(getCenterX + (camDisplaceX * 15), getCenterY);
	...
	override function stepHit()
	{
		...
		if (SONG.song.toLowerCase() == 'The Lost Tree')
		{
			if (curStep == 324) // start
			{
				...
				FlxTween.tween(FlxG.camera, {zoom: 1.4}, 1.2, {
					ease: FlxEase.expoOut
				});
			}
			if (curStep == 332)
